function create_synth_image(id, type)
    ...
    if exist(fullfile(data_dir, nm_file), 'file')
        nii_t1w = read_nifti_image(fullfile(data_dir, t1w_file));
        img_t1w = nii_t1w.img;

        nii_nm = read_nifti_image(fullfile(data_dir, nm_file));
        img_nm = nii_nm.img;

        nii_synth_nm_background_weight = read_nifti_image(fullfile(data_dir, synth_nm_background_weight_file));
        nii_synth_nm_brainstem_weight = read_nifti_image(fullfile(data_dir, synth_nm_brainstem_weight_file));
        nii_synth_nm_l_sn_weight = read_nifti_image(fullfile(data_dir, synth_nm_l_sn_weight_file));
        nii_synth_nm_r_sn_weight = read_nifti_image(fullfile(data_dir, synth_nm_r_sn_weight_file));
